...
<head>
    ...
    <title>Assistente FF – Módulo de Otimização V4.0</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <style>
        /* ========================================================= */
        /* VARIÁVEIS DE COR E ESTILO - TEMA CYBERPUNK (ROXO/CIANO)   */
        /* ========================================================= */
        :root {
        ...
        /* Fundo de Vídeo e Efeito de Matriz */
        #video-background { 
        ...
        /* Efeito de Scanline/Matrix (simulação de código no fundo) */
        .matrix-overlay {
            position: fixed;
        ...
        /* Estilo para as caixas de função e diagnóstico */
        .module-box {
            padding: 1.5rem;
        ...
        /* Estilos para a parte de Conexão/Ping */
        .ping-display {
            font-size: 3rem;
            font-weight: bold;
        ...
        /* Estilo do Botão Principal ATIVAR */
        .activate-button {
        ...
        /* ========================================================= */
        /* ESTILOS DE LOGIN (MANTIDOS E ADAPTADOS) */
        /* ========================================================= */
...
<body class="futuristic-theme" id="main-body">
    ...
    <div id="login-screen" class="login-screen-container">
        <div class="login-box">
            ...
            <h2 class="text-xl text-gray-300 mb-8 font-light border-b-2 border-neon-secondary/30 pb-3">
                <span class="text-neon-secondary font-bold">MÓDULO DE AUTENTICAÇÃO HFR-42</span>
            </h2>

            <form id="login-form" onsubmit="handleLogin(event)">
                <label for="key-input" class="block text-left text-sm font-semibold text-gray-200 mb-2">
                    🔑 CHAVE DE ACESSO EXCLUSIVA:
                </label>
    ...
    <div id="key-modal" class="modal-key-dialog hidden">
        <div class="modal-key-content">
            <h3 class="text-3xl font-bold text-neon-primary mb-4">🔑 STATUS DA CHAVE DE ACESSO</h3>
            
            <p class="text-gray-300 mb-4" id="key-modal-info">Verificando expiração...</p>
            
            <div class="key-display-container">
                <span id="current-valid-key" class="key-display">CHAVE NÃO ENCONTRADA</span>
                <button onclick="copyKeyToClipboard()" class="copy-button" id="copy-btn">
                    <span id="copy-text">📋 COPIAR</span>
                </button>
            </div>

            <p class="text-sm text-yellow-400 mt-2">AVISO: Esta chave está vinculada a este dispositivo/navegador até a data de expiração.</p>
    ...
    <div id="admin-modal" class="modal-key-dialog hidden">
        <div class="modal-key-content">
            <h3 class="text-3xl font-bold text-red-500 mb-4">👑 PAINEL DE GESTÃO DE CHAVES</h3>
            <p class="text-yellow-400 mb-4">ACESSO CONCEDIDO: Admin Master Key ATIVA.</p>
            
            <div class="space-y-4 text-left">
                <label class="block text-gray-200 font-bold">1. SELECIONE A DURAÇÃO DA NOVA KEY (Para o cliente):</label>
                ...
                <button onclick="adminGenerateNewKey()" class="key-submit-button bg-red-600 hover:bg-red-700 text-2xl mt-4">
                    ⚠️ GERAR NOVA KEY COM DURAÇÃO
                </button>
                <p id="admin-message" class="error-message text-center mt-2"></p>
                <p class="text-xs text-gray-500 text-center">A nova chave será automaticamente copiada para sua área de transferência.</p>
            </div>
        ...
        <div class="hud-frame">
            <header class="text-center mb-10">
                ...
                <h2 class="text-xl font-light text-gray-300 mt-2">
                    MÓDULO DE OTIMIZAÇÃO E ASSISTÊNCIA V4.0
                </h2>
            </header>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    <h3 class="text-2xl font-bold text-neon-secondary mb-4 border-b border-neon-secondary/50 pb-2">FUNÇÕES EXCLUSIVAS</h3>

                    <div class="module-box flex items-center justify-between mb-4">
                        <label for="toggle-optimization-input" class="text-xl font-medium text-gray-100">Otimização de Ping</label>
                    ...
                    <div class="module-box flex items-center justify-between mb-4">
                        <label for="toggle-precision-input" class="text-xl font-medium text-gray-100">Anti-Recoil Avançado</label>
                    ...
                    <div class="module-box flex items-center justify-between mb-8">
                        <label for="toggle-aimguide-input" class="text-xl font-medium text-gray-100">Assistência de Mira (AimGuide)</label>
                    ...
                    <h3 class="text-xl font-bold text-neon-primary mb-3">SELEÇÃO DE DPI: <span id="dpi-value" class="text-neon-secondary">50%</span></h3>
                ...
                <div class="space-y-6">
                    ...
                    <div class="module-box border-l-4 border-neon-secondary">
                        <h4 class="text-lg font-bold text-neon-secondary mb-3">DIAGNÓSTICO EM TEMPO REAL</h4>
                        <p class="text-xl font-medium text-gray-100 mb-2">STATUS CONEXÃO</p>
                    ...
                    <div class="module-box border-l-4 border-neon-primary">
                        <h4 class="text-lg font-bold text-neon-primary mb-3">DETECÇÃO DE ERROS (V.O.L.S)</h4>
            ...
            <div class="key-info-footer">
                ...
                <button onclick="displayCurrentKey()" class="copy-key-icon">
                    <span id="copy-text-footer">VER EXPIRAÇÃO</span>
                </button>
            </div>
    ...
    <script>
        ...
        // =========================================================
        // DEFINIÇÕES DE SEGURANÇA E ADMIN
        // =========================================================
        ...
        // --- FUNÇÕES DE GERAÇÃO E VALIDAÇÃO DE KEY COM EXPIRAÇÃO ---
        ...
        async function handleLogin(event) {
            ...
            // Lógica Central: Se a chave digitada for a MESTRE (para o cliente), armazena ela e loga.
            // Nota: No nosso sistema front-end, a "chave mestra" para o cliente é a última chave gerada pelo Admin.
            // Para simular a entrega de uma nova key, trataremos qualquer key 6-6-6 como "nova" se não houver uma válida.
            if (!keyMatches && isKeyFormatValid(enteredKey)) {
                 // Assumindo que o Admin entregou essa chave, a armazenamos localmente.
                 generateNewAccessKey(7); // default 7 dias, mas idealmente teria uma duração vinculada.
                 const keyData = JSON.parse(localStorage.getItem(KEY_STORAGE_ID));
                 keyData.key = enteredKey;
                 // Definir uma expiração padrão para a nova chave (ex: 7 dias a partir de agora)
            ...
            if (keyMatches && isExpired) {
                ...
                loginErrorMsg.textContent = 'ERRO [COD 403]: Chave inválida, expirada ou não reconhecida. Tente novamente.';
            }
        ...
        function completeLogin() {
            ...
            loadAppState(); 
            initializeApp();
            updateKeyInfoInFooter(); // Atualiza o rodapé com a Key
        }
        ...
        function copyKeyToClipboard() {
            ...
            if (key && key !== 'CHAVE NÃO ENCONTRADA') {
        ...
        function updateKeyInfoInFooter() {
            ...
            if (keyData && keyData.key) {
                ...
                keyFooter.textContent = 'KEY: [N/A] (FAÇA LOGIN)';
            }
        }
            ...
            if (storedKeyJSON) {
                try {
                    ...
                    if (Date.now() >= keyData.expiry) {
                         ...
                         keyModalInfo.innerHTML = `DURAÇÃO: ${keyData.durationDays} dias | Válida até: <span class="text-neon-secondary font-bold">${formattedDate}</span>`;
                    }
                } catch (e) {
                ...
                keyDisplayElement.textContent = 'CHAVE NÃO ENCONTRADA';
                keyModalInfo.innerHTML = '<span class="text-red-500 font-bold">Status: Não logado.</span>';
            }
        ...
        function adminGenerateNewKey() {
            ...
            adminMsg.textContent = `✅ NOVA KEY GERADA: ${newKey} (Duração: ${durationDays} dias). COPIADA!`;
            adminMsg.style.color = '#00eaff'; 
            adminMsg.style.textShadow = '0 0 5px rgba(0, 234, 255, 0.8)';
            
            navigator.clipboard.writeText(newKey);
        }
        // --- FIM FUNÇÕES DE ADMIN ---

        // --- FUNÇÕES DE PERSISTÊNCIA E SIMULAÇÃO ---
        ...
        function toggleFeature(feature, isLoad = false) {
            ...
            if (activeCount > 0) {
                statusElement.innerHTML = `MODOS ATIVOS: ${activeCount}. DIAGNÓSTICO EM CURSO. 
        ...
        function startPingSim() {
            ...
            pingInterval = setInterval(() => {
                ...
                // Atualiza o histórico (normaliza o ping para a altura do gráfico 0-50)
        ...
        function activateModsAndLaunch() {
            // Lógica de ativação e lançamento
            const activeCount = Object.values(appState).filter(val => val === true).length;
            if (activeCount > 0) {
                alert(`ATIVANDO: ${activeCount} MODS. INICIANDO FREE FIRE OTIMIZADO...`);
            } else {
                alert("Nenhuma função ativada. Iniciando Free Fire normalmente...");
            }
            
            // Simula o lançamento do aplicativo (Deep Link)
        ...
        // Chama a função de verificação de login ao carregar
        document.addEventListener('DOMContentLoaded', checkLoginStatus);
        
        // Função para checar o login (mantida da v3.0)
